-- CSBwin to DSB Conversion script version 1.6
--
...
if (CONVERTING_FROM == "DM") then
	...
	CSB2DSB["Water Flask"] = "flask_water"
...
arch_names = {
	...
	basic_door_strength = { 110, 42, 230, 255 },
	deco_doors = { },
...
convert = {
				...
				if (not dsb_get_gfxflag(x, GF_INACTIVE)) then
					...
					if (bl_len > 1) then
						...
						if (exvar[x].OBJECT_TRIGGER) then
							...
							-- Reverse const_weight off triggers
							if (exvar[x].const_weight and exvar[x].disable_self) then
								exvar[x].off_trigger = true
								exvar[x].const_weight = nil
								if (exvar[x].msg == M_ACTIVATE) then
									exvar[x].msg = M_DEACTIVATE
								elseif (exvar[x].msg == M_DEACTIVATE) then
		...
		while (itemlist and (itemlist ~= EMPTY)) do
			...
			if (here ~= nil) then
				...
				if (ptable.roomType == "DOOR" and here.door == true) then
					...
					local door_power = arch_names.basic_door_strength[doortype_id]
	...
	tbl_pt_to_exvars = function(h, lev, x, y)
		...
		if (button_type == "SwitchAnything") then
			object_opby = false
		elseif (button_type == "SwitchFullHand") then
			object_opby = false
			exv.opby_thing = true
		elseif (button_type == "SwitchEmptyHand") then
			object_opby = false
